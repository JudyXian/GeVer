...
def parse_option():
    parser = argparse.ArgumentParser(
        "command line arguments for recall columns")
    ...
    for tab, cols in tabs_cols.items():
        ...
        for tab_ori in data['db_schema']:
            if tab == tab_ori['table_name_original'].lower():
                cols_ori = [item.lower()
                            for item in tab_ori['column_names_original']]
...
def c3_column_recall(input_recalled_tables_path,output_recalled_columns_path):
    with open(input_recalled_tables_path) as f:
        data_all = json.load(f)
        # data_all = data_all[773:]
    res = []
    sc_num = 10
    # 选择与问题最相关的column
    for i, data in enumerate(tqdm(data_all)):
        schema = generate_schema(data) #这里的table 是经过table_recall.py 过滤后的tables
        ...
        tab_col_ori = {}
        for table in data['db_schema']:
            tab_col_ori[table['table_name_original'].lower(
