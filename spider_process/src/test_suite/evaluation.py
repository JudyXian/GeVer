...
def test_suite_evaluation(gold, predict, db_dir, etype,exec_result_path,plug_value=False,kmaps=None,keep_distinct=False,progress_bar_for_each_datapoint=False):
    score = []
    with open(gold) as f:
        glist = []
        gseq_one = []
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "exec"]:
                exec_score,exec_result = eval_exec_match(db=db, p_str=p_str, g_str=g_str, plug_value=plug_value,
                                             keep_distinct=keep_distinct, progress_bar_for_each_datapoint=progress_bar_for_each_datapoint)
                # xyr
                all_exec_result.append(exec_result)
                # 把extra的分数输出（xyr）
                # if hardness == "extra":
                # if exec_score == 0:
                #     print(xyr_index)
        ...
        # xyr
        with open(exec_result_path, mode='w') as f:
            for i in all_exec_result:
                f.write(str(i) + '\n')
        print(score)
        print(len(score))
        #
...
def test_suite_evaluation_with_error_analysis(gold, predict, json_file,output_error_file,db_dir, etype,exec_result_path,plug_value=False,kmaps=None,keep_distinct=False,progress_bar_for_each_datapoint=False):
    score = []
    errors = []
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for data_index,data_json in enumerate(json_infos):
            if score[data_index]==0:
                data_json['pred'] = p_str
                data_json['exec_score'] = 0
                errors.append(data_json)
