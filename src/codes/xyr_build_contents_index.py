...
def build_content_index(db_path, index_path):
    ...
    for table_name in table_names:
        ...
        for column_name in column_names_in_one_table:
            try:
                print("SELECT DISTINCT `{}` FROM `{}` WHERE `{}` IS NOT NULL;".format(
                    column_name, table_name, column_name))
                results = execute_sql_long_time_limitation(
                    cursor, "SELECT DISTINCT `{}` FROM `{}` WHERE `{}` IS NOT NULL;".format(column_name, table_name, column_name))
                column_contents = [str(result[0]).strip()
...
if __name__ == "__main__":
    ...
    #     build_content_index(
    #         os.path.join(
    #             "./data/sft_data_collections/bird/dev/dev_databases/", db_id, db_id + ".sqlite"),
    #         os.path.join(
    ...
    for db_id in os.listdir("./data/sft_data_collections/bird/train/train_databases"):
        ...
        build_content_index(
            os.path.join(
                "./data/sft_data_collections/bird/train/train_databases/", db_id, db_id + ".sqlite"),
            os.path.join(
