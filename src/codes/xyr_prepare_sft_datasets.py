...
def extract_large_numbers(text):
    ...
    for word, multiplier in patterns.items():
        ...
        for match in matches:
            ...
            number_information.append(
                match + " " + word + " = " + str(int(number)))
...
def remove_table_alias(s):
    ...
    for i in range(1, 11):
        if "t{}".format(i) in tables_aliases.keys():
            new_tables_aliases["t{}".format(
                i)] = tables_aliases["t{}".format(i)]
...
def str_replace_ignore_case(evidence, schema_item_name):
    evidence = re.sub(re.escape(schema_item_name),
                      schema_item_name, evidence, 0, re.IGNORECASE)
...
def spider_style_dataset(
    ...
    for db_info in additional_db_info:
        ...
        column_names = [column_name.lower()
                        for _, column_name in db_info["column_names_original"]]
        table_idx_of_each_column = [t_idx for t_idx,
                                    _ in db_info["column_names_original"]]
        column_comments = [column_comment.lower()
                           for _, column_comment in db_info["column_names"]]

        assert len(column_names) == len(column_comments)
        column_comments = remove_similar_comments(
            column_names, column_comments)

        table_names = [table_name.lower()
                       for table_name in db_info["table_names_original"]]
        table_comments = [table_comment.lower()
                          for table_comment in db_info["table_names"]]
    ...
    for data in dataset:
        ...
        if db_id in db_id2schema:
            ...
            db_id2schema[db_id] = get_db_schema(
                sample["db_path"], db_comments, db_id)
            sample["schema"] = db_id2schema[db_id]
        ...
        if mode in ["train", "dev"]:
            sql = data["SQL"] if source in [
                "bird-dev", "bird-train"] else data["query"]
            sample["sql"] = remove_table_alias(sqlparse.format(
        ...
        try:
            sql_tokens = [token.value for token in Parser(
                sample["sql"].lower()).tokens]
        except Exception as e:
        ...
        for table_info in sample["schema"]["schema_items"]:
            if mode in ["train", "dev"]:
                ...
                sample["table_labels"].append(
                    1 if table_name in sql_tokens else 0)
        ...
        for i in range(len(hits)):
            ...
            if tc_name in coarse_matched_contents.keys():
                if matched_result["contents"] not in coarse_matched_contents[tc_name]:
                    coarse_matched_contents[tc_name].append(
            ...
            for _match_str, (field_value, _s_match_str, match_score, s_match_score, _match_size,) in fm_contents:
                ...
                if tc_name in fine_matched_contents.keys():
                    if len(fine_matched_contents[tc_name]) < 25:
                        fine_matched_contents[tc_name].append(
                            field_value.strip())
                else:
...
if __name__ == "__main__":
    ...
    bird_dev = spider_style_dataset(
        dataset_path="./data/sft_data_collections/bird/dev/0_ratio_dev_200.json",
