...
def normalization(sql):
    ...
    def processing_func(x): return remove_table_alias(
...
def c3_preprocess(mode, table_path,input_dataset_path,output_dataset_path,db_path,target_type):
    
    dataset = json.load(open(input_dataset_path))
    all_db_infos = json.load(open(table_path))

    # xyr
    for i in all_db_infos:
        new_pk = []
        for j in i["primary_keys"]:
            if type(j) == list:
                for k in j:
                    new_pk.append(k)
            else:
                new_pk.append(j)
        i["primary_keys"] = new_pk
    ###
    ...
    for natsql_data, data in tqdm(zip(natsql_dataset, dataset)):
        ...
        if mode == "test":
            ...
            sql_skeleton = extract_skeleton(
                norm_sql, db_schemas[db_id]).strip()
            ...
            if natsql_data is not None:
                ...
                natsql_skeleton = extract_skeleton(
                    norm_natsql, db_schemas[db_id]).strip()
                natsql_used_columns = [
                    token for token in norm_natsql.split() if "." in token and token != "@.@"]
                natsql_tokens = []
