...
def xyr_sql2nl_verify(root_path, model):
    ...
    # output_path = root_path+"verified_sql.txt"
    # with open(output_path, 'w') as f:
    #     for i in pred_sql:
    #         f.write(i+"\n")
...
if __name__ == "__main__":
    ...
    # output_path = root_path+f"1_bridge_from_decomposition_{model}.json"
    # decomposition_question(input_path, output_path, model, prompt_num)
    # prompt_num = 1  # bridge-select
    # output_path = root_path+f"1_bridge_select_decomposition_{model}.json"
    # decomposition_question(input_path, output_path, model, prompt_num)
    # prompt_num = 2  # bridge-where
    # output_path = root_path+f"1_bridge_where_decomposition_{model}.json"
    # decomposition_question(input_path, output_path, model, prompt_num)
    # prompt_num = 3  # combination
    # output_path = root_path+f"1_combination_decomposition_{model}.json"
    # decomposition_question(input_path, output_path, model, prompt_num)

    # # 2.1.处理数据为dail可处理格式
    # decomposition_result_file = root_path + \
    #     f"1_bridge_from_decomposition_{model}.json"
    # processed_file = root_path+"2.1_bridge_from_for_dail.json"
    # process_data_for_dail(decomposition_result_file, processed_file)

    # decomposition_result_file = root_path + \
    #     f"1_bridge_select_decomposition_{model}.json"
    # processed_file = root_path+"2.1_bridge_select_for_dail.json"
    # process_data_for_dail(decomposition_result_file, processed_file)

    # decomposition_result_file = root_path + \
    #     f"1_bridge_where_decomposition_{model}.json"
    # processed_file = root_path+"2.1_bridge_where_for_dail.json"
    # process_data_for_dail(decomposition_result_file, processed_file)

    # decomposition_result_file = root_path + \
    #     f"1_combination_decomposition_{model}.json"
    # processed_file = root_path+"2.1_combination_for_dail.json"
    # process_data_for_dail(decomposition_result_file, processed_file)

    # # 2.2.处理最后一个子问题为可处理格式
    # input_path = root_path + f"1_bridge_from_decomposition_{model}.json"
    # output_path = root_path + "2.2_bridge_from.json"
    # process_last_question(input_path, output_path)

    # input_path = root_path + f"1_bridge_select_decomposition_{model}.json"
    # output_path = root_path + "2.2_bridge_select.json"
    # process_last_question(input_path, output_path)

    # input_path = root_path + f"1_bridge_where_decomposition_{model}.json"
    # output_path = root_path + "2.2_bridge_where.json"
    # process_last_question(input_path, output_path)

    # input_path = root_path + f"1_combination_decomposition_{model}.json"
    # output_path = root_path + "2.2_combination.json"
    # process_last_question(input_path, output_path)

    # # 3.2 使用prompt生成最后一个子SQL
    # input_path = root_path + "2.2_bridge_from.json"
    # output_path = root_path + f"3.2_bridge_from_{model}.txt"
    # last_sql_generate(input_path, output_path,model)

    # input_path = root_path + "2.2_bridge_select.json"
    # output_path = root_path + f"3.2_bridge_select_{model}.txt"
    # last_sql_generate(input_path, output_path,model)

    # input_path = root_path + "2.2_bridge_where.json"
    # output_path = root_path + f"3.2_bridge_where_{model}.txt"
    # last_sql_generate(input_path, output_path,model)

    # input_path = root_path + "2.2_combination.json"
    # output_path = root_path + f"3.2_combination_{model}.txt"
    # last_sql_generate(input_path, output_path,model)

    # # 4.1.处理dail生成结果
    # processed_file = root_path+"2.1_bridge_from_for_dail.json"
    # dail_result = root_path+f"3.1_bridge_from_{model}.txt"
    # output_processed_file = root_path+"4_bridge_from.json"
    # process_dail_result(processed_file, dail_result, output_processed_file)

    # processed_file = root_path+"2.1_bridge_where_for_dail.json"
    # dail_result = root_path+f"3.1_bridge_where_{model}.txt"
    # output_processed_file = root_path+"4_bridge_where.json"
    # process_dail_result(processed_file, dail_result, output_processed_file)

    # processed_file = root_path+"2.1_bridge_select_for_dail.json"
    # dail_result = root_path+f"3.1_bridge_select_{model}.txt"
    # output_processed_file = root_path+"4_bridge_select.json"
    # process_dail_result(processed_file, dail_result, output_processed_file)

    # processed_file = root_path+"2.1_combination_for_dail.json"
    # dail_result = root_path+f"3.1_combination_{model}.txt"
    ...
    # lastsql_result = root_path + f"3.2_bridge_from_{model}.txt"
    # process_last_sql(dail_file, lastsql_info, lastsql_result)

    # dail_file = root_path + "4_bridge_select.json"
    # lastsql_info = root_path + "2.2_bridge_select.json"
    # lastsql_result = root_path + f"3.2_bridge_select_{model}.txt"
    # process_last_sql(dail_file, lastsql_info, lastsql_result)

    # dail_file = root_path + "4_bridge_where.json"
    # lastsql_info = root_path + "2.2_bridge_where.json"
    # lastsql_result = root_path + f"3.2_bridge_where_{model}.txt"
    # process_last_sql(dail_file, lastsql_info, lastsql_result)

    # dail_file = root_path + "4_combination.json"
    # lastsql_info = root_path + "2.2_combination.json"
    # lastsql_result = root_path + f"3.2_combination_{model}.txt"
    # process_last_sql(dail_file, lastsql_info, lastsql_result)

    # # 5.使用gpt生成最终sql
    # processed_dail_result = root_path+"4_bridge_from.json"
    # output_path = root_path+f"5_bridge_from_{model}.json"
    # composite_final_SQL(processed_dail_result, output_path,model)

    # processed_dail_result = root_path+"4_bridge_where.json"
    # output_path = root_path+f"5_bridge_where_{model}.json"
    # composite_final_SQL(processed_dail_result, output_path,model)

    # processed_dail_result = root_path+"4_bridge_select.json"
    # output_path = root_path+f"5_bridge_select_{model}.json"
    # composite_final_SQL(processed_dail_result, output_path,model)

    # processed_dail_result = root_path+"4_combination.json"
    # output_path = root_path+f"5_combination_{model}.json"
    # composite_final_SQL(processed_dail_result, output_path,model)
    ...
    # # 6.生成供评估的gold与pre
    # final_result = root_path+f"5_bridge_from_{model}.json"
    ...
    # final_result = root_path+f"5_bridge_where_{model}.json"
    # eval_gold = root_path+"6_gold_bridge_where.txt"
    # eval_pred = root_path+"6_pred_bridge_where.txt"
    # process_data_for_eval(final_result, eval_gold, eval_pred)

    # final_result = root_path+f"5_bridge_select_{model}.json"
    # eval_gold = root_path+"6_gold_bridge_select.txt"
    # eval_pred = root_path+"6_pred_bridge_select.txt"
    # process_data_for_eval(final_result, eval_gold, eval_pred)

    # final_result = root_path+f"5_combination_{model}.json"
    # eval_gold = root_path+"6_gold_combination.txt"
    # eval_pred = root_path+"6_pred_combination.txt"
    # process_data_for_eval(final_result, eval_gold, eval_pred)

    test_suite_evaluation(db_dir="/text2sql/spider/database", gold=root_path+"6_real_gold.txt", predict=root_path +
                          "6_pred_bridge_from.txt", etype="exec", exec_result_path=root_path+"bridge_from_exec.txt")
    # test_suite_evaluation(db_dir="/text2sql/spider/database",gold=root_path+"6_real_gold.txt",predict=root_path+"6_pred_bridge_select.txt",etype="exec",exec_result_path=root_path+"bridge_select_exec.txt")
    # test_suite_evaluation(db_dir="/text2sql/spider/database",gold=root_path+"6_real_gold.txt",predict=root_path+"6_pred_bridge_where.txt",etype="exec",exec_result_path=root_path+"bridge_where_exec.txt")
    # test_suite_evaluation(db_dir="/text2sql/spider/database",gold=root_path+"6_real_gold.txt",predict=root_path+"6_pred_combination.txt",etype="exec",exec_result_path=root_path+"combination_exec.txt")
    # test_suite_evaluation(db_dir="/text2sql/spider/database",gold=root_path+"gold_new_2000.txt",predict=root_path+"3_new_2000.txt",etype="exec",exec_result_path=root_path+"new_2000_exec.txt")
    # 7.把5加上执行结果
    eval_score_process(root_path, model)

    # 8.统计正确率
    # analysis_acc(root_path,model)
    # lever_verify_acc(root_path)
    # xyr_verify_acc(root_path)
    # test_suite_evaluation(db_dir="/text2sql/spider/database",gold=root_path+"6_gold_combination.txt",predict=root_path+"xyr_bert_verified_sql.txt",etype="exec",exec_result_path=root_path+"bert_verified_sql_exec.txt")
    # xyr_choose_correct_new(root_path)
    # #以下为sql2nl部分
    # xyr_sql2nl_verify(root_path,model)
    # xyr_revision(root_path,model)
    # xyr_revision_with_schema(root_path)
    # 最新的verify+revision
    # xyr_classified_revision(root_path,model)
